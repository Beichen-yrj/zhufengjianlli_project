    ...
    private String buildPolishPrompt(String content, String type) {
        ...
        if ("extract".equals(type)) {
            return """
                ...
                【板块限定规则 — 必须严格遵守】
                请先检查文本开头是否包含方括号标记。如果包含，则只提取到指定板块，JSON 中只返回该板块的字段，其他所有字段一律不返回（设为空或不输出）。

                标记与板块的对应关系：
                - [工作经历] / [工作] / [经历] → 只返回 experience 字段（数组）
                - [教育经历] / [教育] / [学历] → 只返回 education 字段（数组）
                - [项目经历] / [项目] → 只返回 projects 字段（数组）
                - [基本信息] / [个人] / [基本] → 只返回 basic 字段（对象）
                - [专业技能] / [技能] → 只返回 skills 字段（字符串）
                - [自我评价] / [评价] / [自我] → 只返回 selfEvaluation 字段（字符串）
                - [证书荣誉] / [获奖经历] / [证书] / [奖项] / [荣誉] → 只返回 certificates 字段（数组），每项的 description 字段填写获奖经历描述

                判定规则：
                1. 检查文本最开头（忽略空白后）是否匹配上述任一标记
                2. 匹配成功 → 仅提取并返回该标记对应的单个字段，其余字段全部省略
                3. 无匹配标记或无标记 → 正常全量提取所有有信息的字段

                ⚠️ 关键约束：当检测到板块标记时，如果该板块没有有效信息可提取，返回一个空数组或空字符串表示"未提取到内容"，绝对不要将数据填入其他板块。
        ...
        return """
            你是一位拥有10年经验的资深简历润色工程师，专门帮助求职者优化简历内容，提升面试通过率。%s

            润色原则：
            1. 使用 STAR 法则（情境、任务、行动、结果）优化经历描述
            2. 量化成果（如"提升30%%"、"管理5人团队"、"节省成本50万"）
            3. 使用行业关键词和动作动词（如"主导""构建""优化""推动"）
            4. 保持原有语言风格和真实性，不夸大不虚构
            5. 精简冗余表述，突出核心亮点
            6. 直接返回润色后的完整文本，不要任何解释或额外文字
