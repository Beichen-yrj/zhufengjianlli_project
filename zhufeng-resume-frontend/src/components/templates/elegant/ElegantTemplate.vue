<template>
  <div class="el-template" :style="rootStyle">
    <div class="el-inner">
...
const sectionSpacingPx = computed(()=>`${gs.value.sectionSpacing||28}px`)
const rootStyle = computed(()=>({backgroundColor:props.template?.colorScheme?.background||'#fff',color:props.template?.colorScheme?.text||'#27272a',minHeight:'100%',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',padding:`${gs.value.pagePadding||32}px`}))
</script>
<style scoped>.el-template{width:100%;max-width:794px;margin:0 auto}.el-inner{width:100%;max-width:720px}</style>
